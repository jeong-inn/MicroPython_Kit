...
def display_oled() :
    solar_voltage_value = Pin(A3).value()
    print("태양광 센서 : ", solar_voltage_value)

    text1[32] = "S : "
    value1[32]
    str1 = str(solar_voltage_value * c_value, DEC)
    
    text1 = text1 + value1
    text1 = text1 + "V"
    list1 = list(str1)
    windturbine_voltage_value = Pin(A5).value
    print("풍력 센서 : ", windturbine_voltage_value)
    print("-------------------");

    text2[32] = "W : "
    value2[32]
    str2 = Str(windturbine_voltage_value * c_value, DEC)
    list2 = list(str2)
    text2 = text2 + value2
    text2 = text2 + "V"
